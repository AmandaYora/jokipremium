...
DROP POLICY IF EXISTS "Admins can view all analytics pageviews" ON public.analytics_pageviews;
DROP POLICY IF EXISTS "Admins can view all analytics section focus" ON public.analytics_section_focus;
...
CREATE INDEX idx_analytics_sessions_network_prefix ON public.analytics_sessions(network_prefix);
...
CREATE TABLE public.analytics_section_focus (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID REFERENCES public.analytics_sessions(id) ON DELETE CASCADE,
...
CREATE INDEX idx_analytics_section_focus_created_at ON public.analytics_section_focus(created_at);
CREATE INDEX idx_analytics_section_focus_session_id ON public.analytics_section_focus(session_id);
CREATE INDEX idx_analytics_section_focus_date ON public.analytics_section_focus(focus_date);

ALTER TABLE public.analytics_section_focus ENABLE ROW LEVEL SECURITY;

-- RLS Policies: Admin can read
